...
# library(lintr)
# lint(filename = "U://UN-Subnational-Estimates/Rcode/Archived Plots/Report_Plot.R")
rm(list=ls())
...
# retrieve & specify  directories
## JG CHANGE 3 back to 2
...
#### prepare national level models ####
{
  ...
  }else if(gsub("nmr_", "", nmr.filename) %in% 
  ...
  }else if(gsub("u5_", "", u5.filename) %in% list.files("Direct/U5MR/")){
...
#### national betabinomial models ####
{
  ...
  if(nmr.filename %in% list.files("Betabinomial/NMR/")){
    message("Loading natl BB8 NMR results from \n", nmr.filename, ".\n" )
  }else if(gsub("nmr_", "", nmr.filename) %in% 
  ...
  if(u5.filename %in% list.files("Betabinomial/U5MR/")){
    message("Loading natl BB8 U5MR results from \n", u5.filename, ".\n" )
  }else if(gsub("u5_", "", u5.filename) %in% 
  ...
  if(exists('bb.res.natl.unstrat.nmr.allsurveys') & 
     exists('bb.res.natl.unstrat.u5.allsurveys')){
    natl.bb.unstrat.allsurveys.frame <-
      data.frame(lower_nmr = bb.res.natl.unstrat.nmr.allsurveys$overall$lower, 
                 median_nmr = bb.res.natl.unstrat.nmr.allsurveys$overall$median, 
                 upper_nmr = bb.res.natl.unstrat.nmr.allsurveys$overall$upper,
                 lower_u5 = bb.res.natl.unstrat.u5.allsurveys$overall$lower,
                 median_u5 = bb.res.natl.unstrat.u5.allsurveys$overall$median,
                 upper_u5 = bb.res.natl.unstrat.u5.allsurveys$overall$upper,
                 method='natl.bb.unstrat.allsurveys', 
                 years=bb.res.natl.unstrat.nmr.allsurveys$overall$years)
    ...
    natl.bb.unstrat.allsurveys.frame <- 
      data.frame(lower_nmr =NA, median_nmr = NA,
                 ...
                 upper_u5 = bb.res.natl.unstrat.u5.allsurveys$overall$upper,
                 method='natl.bb.unstrat.allsurveys',
                 years=bb.res.natl.unstrat.u5.allsurveys$overall$years)
  }
}
...
{
  nmr.filename <- paste0(country, '_res_admin1_', time.model,
...
##### BB8 admin1 #####
{
  ...
  if(nmr.filename %in% list.files("Betabinomial/NMR/")){
    message("Loading Admin-1 BB8 NMR results from \n", nmr.filename, ".\n" )
  }else if(gsub(paste0("_", time.model),
                "", nmr.filename) %in% 
           list.files("Betabinomial/NMR/")){
    nmr.filename <- gsub(paste0(time.model, "_"), "", nmr.filename)
    message("Loading Admin-1 BB8 NMR results from \n", nmr.filename, ".\n" )
  ...
  if(u5.filename %in% list.files("Betabinomial/U5MR/")){
    message("Loading Admin 1 BB8 U5MR results from \n", u5.filename, ".\n" )
  }else if(gsub(paste0("_", time.model), "", u5.filename) %in% 
           list.files("Betabinomial/U5MR/")){
    u5.filename <- gsub(paste0(time.model, "_"), "", u5.filename)
    message("Loading Admin1 BB8 U5MR results from \n", u5.filename, ".\n" )
  }else{
...
if(exists('poly.layer.adm2')){
  ...
  {
    nmr.filename <- paste0(country, '_res_admin2_', time.model,
                           '_nmr_SmoothedDirect.rda')
  ...
  ##### BB8 admin2 ####
  {
    ...
    ## JG does the unstratified need to be changed to all surveys?
    nmr.filename <- paste0(country, '_res_adm2_', time.model,
                           '_', strata.model, '_nmr.rda')
    
    if(nmr.filename %in% list.files("Betabinomial/NMR/")){
      message("Loading Admin-2 BB8 NMR results from \n", nmr.filename, ".\n" )
    }else if(gsub(paste0("_", time.model),
                  "", nmr.filename) %in% 
             list.files("Betabinomial/NMR/")){
      nmr.filename <- gsub(paste0(time.model, "_"), "", nmr.filename)
    ...
    if(u5.filename %in% list.files("Betabinomial/U5MR/")){
      message("Loading Admin 2 BB8 U5MR results from \n", u5.filename, ".\n" )
    }else if(gsub(paste0("_", time.model), "", u5.filename) %in% 
             list.files("Betabinomial/U5MR/")){
      u5.filename <- gsub(paste0(time.model, "_"), "", u5.filename)
      message("Loading Admin2 BB8 U5MR results from \n", u5.filename, ".\n" )
    }else{
...
methods <- c("natl.sd.frame","sd.adm1.to.natl.frame",
             ...
             'BB8.adm1.unstrat.allsurveys.to.natl.frame',
             "BB8.adm1.strat.bench.to.natl.frame",
             'BB8.adm1.unstrat.allsurveys.bench.to.natl.frame',
             "BB8.adm2.unstrat.to.natl.frame","BB8.adm2.strat.to.natl.frame",
             'BB8.adm2.unstrat.allsurveys.to.natl.frame',
             "BB8.adm2.strat.bench.to.natl.frame",
             'BB8.adm2.unstrat.allsurveys.bench.to.natl.frame',
             "igme.frame")[c(1:5, #natl SD and aggregates
                             6:8, # natl BB8
                             9:11,# admin1 BB8 unbenched
                             12:13,# admin2 BB8 benched,
                             14:17, # admin 2 BB8 unbenched,
                             18:19)] #admin2 bb8 benched]
...
# National y-axis limits ####
y_limits_nmr <- c(min(natl.all$median_nmr,na.rm = T)*1000 - 1,
                  max(natl.all$median_nmr,na.rm = T)*1000 + 1)
y_limits_u5 <- c(min(natl.all$median_u5,na.rm = T)*1000 - 1,
                 max(natl.all$median_u5,na.rm = T)*1000 + 1)
