...
message("If your country does not use GADM shapefiles, ", 
        "you may need to specify this manually.\n")
...

{
  ...
  ## is time model / all files in file string?
...
methods <- c("natl.dir.frame", "natl.sd.frame","sd.adm1.to.natl.frame",
             ...
             "igme.frame")[c(1:6, #natl dir, SD and aggregates
                             7:9, # natl BB8
                             10:12,# admin1 BB8 unbenched
                             13:14,# admin2 BB8 benched,
                             15:17, # admin 2 BB8 unbenched,
...
for(outcome in c("nmr", "u5")){
      ...
      height = 2.67, width = 8)
  {
    ...
    if (nrow(tmp.area) >0 & sum(is.na(tmp.area[, "median"]))
      ...
      polygon(x = c(tmp.area$years, rev(tmp.area$years)),
...
for(outcome in c("nmr", "u5")){
      ...
      height = 9, width = 6)
  {
    ...
    for(area in admin2.names$Internal){
      area.idx <- area.idx + 1
      tmp.area <- tmp_plot[[outcome]][,c("region", "years.num", 
                                         "lower", "upper", "median")] %>% 
        filter(region == area)
      tmp.area$width <- tmp.area$upper - tmp.area$lower
      tmp.area$cex2 <- median(tmp.area$width, na.rm = T)/tmp.area$width
...
bench_str <- ifelse(bench.model == "bench", "_bench", "")
...
for(outcome in c("nmr", "u5")){
  ...
  if(!exists(paste0("Betabinomial/", country, "_adm2_",
...
for(outcome in c("nmr", "u5")){
  ...
  if(!exists(paste0("Betabinomial/", country, "_adm1_",
                    strata.model, "_", time.model, "_",
                    outcome, bench_str, "_postsamp.rda"))){
